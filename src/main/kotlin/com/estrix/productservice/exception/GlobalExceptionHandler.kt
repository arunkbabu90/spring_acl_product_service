...
class GlobalExceptionHandler {
    ...
    fun handleAuthorizationDenied(e: AuthorizationDeniedException, request: WebRequest) =
        ResponseEntity(
            ErrorResponse(
                statusMessage = "Authorization Denied! You don't have permission",
                statusCode = HttpStatus.UNAUTHORIZED.value()
            ), HttpStatus.UNAUTHORIZED
        )
    ...
    fun handleAddProductException(e: AddProductException, request: WebRequest) =
        ResponseEntity(
            ErrorResponse(
                statusMessage = e.message ?: "Failed to add product",
                statusCode = HttpStatus.INTERNAL_SERVER_ERROR.value()
            ), HttpStatus.INTERNAL_SERVER_ERROR
        )
}